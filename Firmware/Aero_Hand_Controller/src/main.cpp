...
void setup() {
  ...
  if (do_homing) {
    ...
    Serial.println("[BOOT] Non-power reset → moving to optimal extend positions");

    int16_t targetPos[7];
    for (int i = 0; i < 7; i++) {
      targetPos[i] = sd[i].extend_count; // 读取修改好的 1934, 3970 等参数
    }

    // 强制执行一次位置同步
    // 注意：这里我们直接用 Serial2 发送，因为此时 gBusMux 可能还没初始化完
    hlscl.SyncWritePosEx((uint8_t*)SERVO_IDS, 7, targetPos, g_speed, g_accel, g_torque);
  }
